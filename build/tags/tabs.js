riot.tag('tabs', '<h2>Tabs</h2> <ul class="list--bare"> <li each="{ tab, i in tabs }" class="{ tab: true, is-active: parent.isActiveTab(tab.ref) }" onclick="{ parent.toggleTab }">{tab.title}</li> </ul> <div class="tabContent"> <div each="{ tab, i in tabs }" class="{ tabContent__item: true, is-active: parent.isActiveTab(tab.ref) }">{tab.content}</div> </div>', function(opts) {
  this.tabs = [
